...
interface OrderItemDetail {
  id: string; productId: string; quantity: number; unitPrice: number; subtotal: number; notes: string; modifiers: string; status: string; destination: string; product: { id: string; name: string; price: number; category: string }
}

interface Order {
  id: string; tableId: string; clientId: string | null; status: string; total: number; subtotal?: number; discount?: number; notes: string; createdById?: string | null; finishedById?: string | null; cancelledById?: string | null; cancelledAt?: string | null; createdAt: string; updatedAt: string; table: { id: string; number: number; zone: string }; client: { id: string; name: string; phone: string; points?: number; visits?: number } | null; items: OrderItemDetail[]; _count?: { items: number }; createdBy?: { id: string; username: string; name: string; role: string } | null; finishedBy?: { id: string; username: string; name: string; role: string } | null; cancelledBy?: { id: string; username: string; name: string; role: string } | null
}
...
const roleLabels: Record<string, string> = {
  ...
  camarero: 'Camarero', cocina: 'Cocinero', barra: 'Barra', caja: 'Caja',
}

const orderStatusConfig: Record<string, { label: string; color: string }> = {
  pending: { label: 'Pendiente', color: 'bg-amber-100 text-amber-800' },
  in_progress: { label: 'En preparación', color: 'bg-orange-100 text-orange-800' },
  ready: { label: 'Listo', color: 'bg-green-100 text-green-800' },
  served: { label: 'Servido', color: 'bg-blue-100 text-blue-800' },
...
function CocinaTab() {
  ...
  const fetchOrders = useCallback(async () => {
    try { const res = await fetch('/api/orders?status=pending,in_progress&destination=kitchen', { headers: authHeaders(false) }); if (handleFetchResponse(res) && res.ok) { const json = await res.json(); setOrders(json.orders) } } catch { /* */ } finally { setLoading(false) }
  }, [authHeaders, handleFetchResponse])
  ...
  const handleItemReady = async (orderId: string, itemId: string) => {
    ...
    try {
      // Mark order as in_progress first if pending
      const order = orders.find(o => o.id === orderId)
      if (order?.status === 'pending') {
        await fetch(`/api/orders/${orderId}`, { method: 'PUT', headers: authHeaders(), body: JSON.stringify({ status: 'in_progress' }) })
      }
      const res = await fetch(`/api/orders/${orderId}/items/${itemId}`, { method: 'PATCH', headers: authHeaders(), body: JSON.stringify({ status: 'ready' }) })
      if (handleFetchResponse(res) && res.ok) {
        const json = await res.json()
        if (json.allReady) {
          toast.success(`Mesa ${order?.table?.number ?? '?'} — ¡Pedido completo listo!`)
        }
        // Refresh to get updated state
        fetchOrders()
      }
    } catch { toast.error('Error de red') } finally { setMarkingItem(null) }
  }
      ...
      {sortedOrders.length === 0 ? <div className="flex flex-col items-center justify-center py-20 text-gray-500"><CheckCheck className="size-16 mb-4" /><p className="text-xl font-medium">¡Todo listo!</p><p className="text-sm mt-1">No hay pedidos pendientes</p></div> : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {sortedOrders.map((order) => (
            <div key={order.id} className={`bg-gray-800 rounded-xl p-4 border-l-4 transition-all ${order.status === 'pending' ? 'border-l-amber-500' : 'border-l-orange-500'}`}>
              ...
              <div className="space-y-1 mb-4">{order.items.map((item) => (
                <div key={item.id} className="flex items-center gap-2">
                  <span className="flex size-6 items-center justify-center rounded bg-amber-600/30 text-amber-300 text-xs font-bold shrink-0">{item.quantity}</span>
                  <span className="text-sm text-white leading-tight flex-1">{item.product?.name ?? 'Producto'}</span>
                  {item.notes && <span className="text-xs text-amber-400">({item.notes})</span>}
                  <Button size="sm" className="h-7 px-2 text-xs bg-green-600 hover:bg-green-700 text-white shrink-0" onClick={() => handleItemReady(order.id, item.id)} disabled={markingItem === item.id}>
                    {markingItem === item.id ? '...' : <><CheckCircle className="size-3 mr-1" />Listo</>}
                  </Button>
                </div>
              ))}</div>
...
function BarraTab() {
  ...
  const fetchOrders = useCallback(async () => {
    try { const res = await fetch('/api/orders?status=pending,in_progress&destination=bar', { headers: authHeaders(false) }); if (handleFetchResponse(res) && res.ok) { const json = await res.json(); setOrders(json.orders) } } catch { /* */ } finally { setLoading(false) }
  }, [authHeaders, handleFetchResponse])
  ...
  if (loading) return <div className="bg-gray-900 rounded-xl p-6 min-h-[60vh] flex items-center justify-center"><div className="flex items-center gap-3 text-gray-400"><Wine className="size-8 animate-pulse" /><span className="text-xl">Cargando pedidos...</span></div></div>

  return (
    <div className="bg-gray-900 rounded-xl p-4 min-h-[calc(100vh-10rem)]">
      <div className="flex items-center justify-between mb-4"><div className="flex items-center gap-3"><Wine className="size-8 text-blue-400" /><h2 className="text-2xl font-bold text-white">Barra</h2><Badge className="bg-blue-600 text-white text-sm">{orders.length} pedidos</Badge></div><Button variant="outline" className="h-10 border-gray-600 text-gray-300 hover:bg-gray-800" onClick={fetchOrders}><ShoppingCart className="size-4 mr-1" />Actualizar</Button></div>
      {sortedOrders.length === 0 ? <div className="flex flex-col items-center justify-center py-20 text-gray-500"><CheckCheck className="size-16 mb-4" /><p className="text-xl font-medium">¡Todo listo!</p><p className="text-sm mt-1">No hay bebidas pendientes</p></div> : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {sortedOrders.map((order) => (
            <div key={order.id} className={`bg-gray-800 rounded-xl p-4 border-l-4 transition-all ${order.status === 'pending' ? 'border-l-blue-500' : 'border-l-cyan-500'}`}>
              <div className="flex items-start justify-between mb-3"><div><p className="text-3xl font-bold text-white">Mesa {order.table?.number ?? '?'}</p><p className="text-sm text-gray-400">{zoneConfig[order.table?.zone]?.label ?? order.table?.zone}</p></div><div className="text-right"><p className="text-sm text-gray-400">{formatTime(order.createdAt)}</p><p className={`text-lg font-bold ${elapsedColor(order.createdAt)}`}><Timer className="size-4 inline mr-1" />{timeAgo(order.createdAt)}</p></div></div>
              <Badge className={`mb-3 ${order.status === 'pending' ? 'bg-blue-600/20 text-blue-400 border-blue-600/30' : 'bg-cyan-600/20 text-cyan-400 border-cyan-600/30'}`} variant="outline">{order.status === 'pending' ? '⏳ Pendiente' : '🍹 En preparación'}</Badge>
              <div className="space-y-1 mb-4">{order.items.map((item) => (
                <div key={item.id} className="flex items-center gap-2">
                  <span className="flex size-6 items-center justify-center rounded bg-blue-600/30 text-blue-300 text-xs font-bold shrink-0">{item.quantity}</span>
                  <span className="text-sm text-white leading-tight flex-1">{item.product?.name ?? 'Producto'}</span>
                  {item.notes && <span className="text-xs text-cyan-400">({item.notes})</span>}
                  <Button size="sm" className="h-7 px-2 text-xs bg-green-600 hover:bg-green-700 text-white shrink-0" onClick={() => handleItemReady(order.id, item.id)} disabled={markingItem === item.id}>
                    {markingItem === item.id ? '...' : <><CheckCircle className="size-3 mr-1" />Listo</>}
                  </Button>
                </div>
...
function RestaurantesTab() {
  ...
  const [deleteTarget, setDeleteTarget] = useState<RestaurantInfo | null>(null)
  ...
  const handleDeleteRestaurant = async () => {
    ...
    setDeleting(true)
    try {
      const res = await fetch(`/api/restaurants/${deleteTarget.id}`, { method: 'DELETE', headers: authHeaders(false) })
      if (res.ok) { toast.success('Restaurante eliminado'); setShowDeleteDialog(false); setDeleteTarget(null); setDeleteConfirmName(''); fetchRestaurants() }
      else { const err = await res.json(); toast.error(err.error || 'Error al eliminar') }
    } catch { toast.error('Error de red') } finally { setDeleting(false) }
  }
  ...
  return (
    <div className="space-y-4">
              ...
              <Card key={r.id} className={`rounded-xl ${!r.active ? 'opacity-60' : ''}`}>
                ...
                <CardContent className="space-y-3">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button variant="outline" size="sm" className="text-red-600 hover:bg-red-50 hover:text-red-700" onClick={() => { setDeleteTarget(r); setShowDeleteDialog(true) }}><Trash2 className="size-4" /></Button>
                  </div>
                </CardContent>
              </Card>
            )
      ...
      <Dialog open={showDeleteDialog} onOpenChange={(open) => { setShowDeleteDialog(open); if (!open) { setDeleteTarget(null); setDeleteConfirmName('') } }}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader><DialogTitle className="flex items-center gap-2"><AlertTriangle className="size-5 text-red-600" />Eliminar Restaurante</DialogTitle></DialogHeader>
          <DialogDescription />
          <div className="space-y-4 py-2">
            <p className="text-sm text-muted-foreground">Esta acción eliminará el restaurante <strong>{deleteTarget?.name}</strong> y todos sus datos asociados (usuarios, mesas, productos, pedidos). Esta acción no se puede deshacer.</p>
            <div className="space-y-2">
              <Label>Escribe <strong>{deleteTarget?.name}</strong> para confirmar</Label>
              <Input placeholder={deleteTarget?.name} value={deleteConfirmName} onChange={(e) => setDeleteConfirmName(e.target.value)} />
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => { setShowDeleteDialog(false); setDeleteTarget(null); setDeleteConfirmName('') }}>Cancelar</Button>
            <Button className="bg-red-600 hover:bg-red-700 text-white" onClick={handleDeleteRestaurant} disabled={deleting || deleteConfirmName !== deleteTarget?.name}>{deleting ? 'Eliminando...' : 'Eliminar Restaurante'}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
...
function UsersTab() {
  ...
  return (
    <div className="space-y-4">
              ...
              {users.map((u) => (
                <tr key={u.id} className={`border-b hover:bg-muted/30 ${!u.active ? 'opacity-50' : ''}`}>
                  ...
                  <td className="p-3 text-right"><div className="flex gap-1 justify-end">
                    ...
                    <Button variant="ghost" size="sm" className="size-8 p-0" onClick={() => { setDeleteUserTarget(u); setShowDeleteUserDialog(true) }} title="Eliminar usuario" disabled={currentUser?.userId === u.id || u.role === 'super_admin'}><Trash2 className="size-4 text-red-500" /></Button>
                  </div></td>
                </tr>
              ))}
            </tbody>
      ...
      {/* Delete User Dialog */}
      <Dialog open={showDeleteUserDialog} onOpenChange={(open) => { setShowDeleteUserDialog(open); if (!open) setDeleteUserTarget(null) }}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader><DialogTitle className="flex items-center gap-2"><AlertTriangle className="size-5 text-red-600" />Eliminar Usuario</DialogTitle></DialogHeader>
          <DialogDescription />
          <div className="space-y-4 py-2">
            <p className="text-sm text-muted-foreground">¿Estás seguro de que quieres eliminar al usuario <strong>{deleteUserTarget?.name}</strong> (<strong>{deleteUserTarget?.username}</strong>)? Esta acción no se puede deshacer.</p>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => { setShowDeleteUserDialog(false); setDeleteUserTarget(null) }}>Cancelar</Button>
            <Button className="bg-red-600 hover:bg-red-700 text-white" onClick={async () => {
              if (!deleteUserTarget) return
              setDeletingUser(true)
              try {
                const res = await fetch(`/api/users/${deleteUserTarget.id}`, { method: 'DELETE', headers: authHeaders(false) })
                if (res.ok) { toast.success('Usuario eliminado'); setShowDeleteUserDialog(false); setDeleteUserTarget(null); fetchUsers() }
                else { const err = await res.json(); toast.error(err.error || 'Error al eliminar usuario') }
              } catch { toast.error('Error de red') } finally { setDeletingUser(false) }
            }} disabled={deletingUser}>{deletingUser ? 'Eliminando...' : 'Eliminar Usuario'}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
...
function ProductsTab() {
  ...
  const [editForm, setEditForm] = useState({ name: '', price: '', category: 'general', stock: '0', description: '', active: true })
  const [showDeleteProductDialog, setShowDeleteProductDialog] = useState(false)
  const [deleteProductTarget, setDeleteProductTarget] = useState<Product | null>(null)
  const [productFilter, setProductFilter] = useState<'all' | 'active' | 'inactive'>('active')

  const fetchProducts = useCallback(async () => {
    try { const res = await fetch('/api/products', { headers: authHeaders(false) }); if (handleFetchResponse(res) && res.ok) { const json = await res.json(); setProducts(json.products) } } catch { /* */ } finally { setLoading(false) }
  }, [authHeaders, handleFetchResponse])
  ...
  const handleEditSave = async () => {
    ...
    if (!editForm.name.trim()) { toast.error('Nombre obligatorio'); return }
    const price = parseFloat(editForm.price)
    if (isNaN(price) || price < 0) { toast.error('Precio inválido'); return }
    try {
      const res = await fetch(`/api/products/${editProduct.id}`, { method: 'PUT', headers: authHeaders(), body: JSON.stringify({ name: editForm.name, price, category: editForm.category, stock: parseInt(editForm.stock) || 0, description: editForm.description, active: editForm.active }) })
      if (res.ok) { toast.success('Producto actualizado'); setShowEditDialog(false); setEditProduct(null); fetchProducts() }
  ...
  const handleDeleteProduct = async () => {
    ...
    try {
      const res = await fetch(`/api/products/${deleteProductTarget.id}`, { method: 'DELETE', headers: authHeaders(false) })
      if (res.ok) { toast.success('Producto eliminado'); setShowDeleteProductDialog(false); setDeleteProductTarget(null); fetchProducts() }
      else { const err = await res.json(); toast.error(err.error || 'Error al eliminar producto') }
    } catch { toast.error('Error de red') }
  }

  const openEditDialog = (p: Product) => {
    setEditProduct(p)
    setEditForm({ name: p.name, price: String(p.price), category: p.category, stock: String(p.stock), description: p.description, active: p.active })
    setShowEditDialog(true)
  }

  const filteredProducts = productFilter === 'all' ? products : productFilter === 'active' ? products.filter((p) => p.active) : products.filter((p) => !p.active)
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex gap-2">
          <div className="flex rounded-lg border overflow-hidden">
            <button className={`px-3 py-1.5 text-sm font-medium transition-colors ${productFilter === 'active' ? 'bg-amber-600 text-white' : 'bg-white text-muted-foreground hover:bg-muted'}`} onClick={() => setProductFilter('active')}>Activos</button>
            <button className={`px-3 py-1.5 text-sm font-medium transition-colors border-x ${productFilter === 'all' ? 'bg-amber-600 text-white' : 'bg-white text-muted-foreground hover:bg-muted'}`} onClick={() => setProductFilter('all')}>Todos</button>
            <button className={`px-3 py-1.5 text-sm font-medium transition-colors ${productFilter === 'inactive' ? 'bg-amber-600 text-white' : 'bg-white text-muted-foreground hover:bg-muted'}`} onClick={() => setProductFilter('inactive')}>Inactivos</button>
          </div>
          <Button className="bg-amber-600 hover:bg-amber-700 text-white" onClick={() => setShowCreateDialog(true)}><Plus className="size-4 mr-1" />Nuevo</Button>
        </div>
      </div>
      {filteredProducts.length === 0 ? <Card className="rounded-xl"><CardContent className="p-6 text-center"><Package className="size-12 mx-auto mb-3 text-muted-foreground" /><p className="text-muted-foreground mb-4">{productFilter === 'inactive' ? 'No hay productos inactivos' : 'No hay productos'}</p><Button className="bg-amber-600 hover:bg-amber-700 text-white" onClick={() => setShowCreateDialog(true)}><Plus className="size-4 mr-1" />Crear Producto</Button></CardContent></Card> : (
        <Card className="rounded-xl"><CardContent className="p-0">
          <ScrollArea className="max-h-[70vh]">
            <table className="w-full text-sm">
              <thead><tr className="border-b bg-muted/50"><th className="text-left p-3">Nombre</th><th className="text-left p-3">Categoría</th><th className="text-right p-3">Precio</th><th className="text-right p-3">Stock</th><th className="text-left p-3">Estado</th><th className="text-right p-3">Acciones</th></tr></thead>
              <tbody>
                {filteredProducts.map((p) => (
                  <tr key={p.id} className={`border-b hover:bg-muted/30 ${!p.active ? 'opacity-50' : ''}`}>
                    <td className="p-3"><div><p className="font-medium">{p.name}</p>{p.description && <p className="text-xs text-muted-foreground">{p.description}</p>}</div></td>
                    <td className="p-3"><Badge variant="outline" className="text-xs">{categoryConfig[p.category]?.label ?? p.category}</Badge></td>
                    <td className="p-3 text-right font-semibold text-amber-700">{formatEUR(p.price)}</td>
                    <td className="p-3 text-right">{p.stock}</td>
                    <td className="p-3"><Badge className={p.active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}>{p.active ? 'Activo' : 'Inactivo'}</Badge></td>
                    <td className="p-3 text-right"><div className="flex gap-1 justify-end">
                      <Button variant="ghost" size="sm" className="size-8 p-0" onClick={() => openEditDialog(p)} title="Editar producto"><Pencil className="size-4 text-amber-600" /></Button>
                      <Button variant="ghost" size="sm" className="size-8 p-0" onClick={() => { setDeleteProductTarget(p); setShowDeleteProductDialog(true) }} title="Eliminar producto"><Trash2 className="size-4 text-red-500" /></Button>
                    </div></td>
                  </tr>
                ))}
              </tbody>
      ...
      {/* Edit Product Dialog */}
      <Dialog open={showEditDialog} onOpenChange={(open) => { setShowEditDialog(open); if (!open) setEditProduct(null) }}>
        <DialogContent><DialogHeader><DialogTitle>Editar Producto</DialogTitle></DialogHeader>
          <div className="space-y-4 py-2">
            <div className="space-y-2"><Label>Nombre *</Label><Input value={editForm.name} onChange={(e) => setEditForm((f) => ({ ...f, name: e.target.value }))} /></div>
            <div className="grid grid-cols-2 gap-3"><div className="space-y-2"><Label>Precio *</Label><Input type="number" step="0.01" value={editForm.price} onChange={(e) => setEditForm((f) => ({ ...f, price: e.target.value }))} /></div><div className="space-y-2"><Label>Stock</Label><Input type="number" value={editForm.stock} onChange={(e) => setEditForm((f) => ({ ...f, stock: e.target.value }))} /></div></div>
            <div className="space-y-2"><Label>Categoría</Label><Select value={editForm.category} onValueChange={(v) => setEditForm((f) => ({ ...f, category: v }))}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{categoryOrder.map((c) => <SelectItem key={c} value={c}>{categoryConfig[c]?.label ?? c}</SelectItem>)}</SelectContent></Select></div>
            <div className="space-y-2"><Label>Descripción</Label><Input value={editForm.description} onChange={(e) => setEditForm((f) => ({ ...f, description: e.target.value }))} /></div>
            <div className="flex items-center gap-2"><input type="checkbox" id="editActive" checked={editForm.active} onChange={(e) => setEditForm((f) => ({ ...f, active: e.target.checked }))} className="size-4 rounded border-gray-300" /><Label htmlFor="editActive" className="cursor-pointer">Producto activo</Label></div>
          </div>
          <DialogFooter><Button variant="outline" onClick={() => { setShowEditDialog(false); setEditProduct(null) }}>Cancelar</Button><Button className="bg-amber-600 hover:bg-amber-700 text-white" onClick={handleEditSave}>Guardar</Button></DialogFooter>
        </DialogContent>
      </Dialog>

      {/* Delete Product Dialog */}
      <Dialog open={showDeleteProductDialog} onOpenChange={(open) => { setShowDeleteProductDialog(open); if (!open) setDeleteProductTarget(null) }}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader><DialogTitle className="flex items-center gap-2"><AlertTriangle className="size-5 text-red-600" />Eliminar Producto</DialogTitle></DialogHeader>
          <DialogDescription />
          <div className="space-y-4 py-2">
            <p className="text-sm text-muted-foreground">¿Estás seguro de que quieres eliminar el producto <strong>{deleteProductTarget?.name}</strong>? El producto será marcado como inactivo.</p>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => { setShowDeleteProductDialog(false); setDeleteProductTarget(null) }}>Cancelar</Button>
            <Button className="bg-red-600 hover:bg-red-700 text-white" onClick={handleDeleteProduct}>Eliminar</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
...
function OrdersTab() {
  ...
  const [cancelledDateFilter, setCancelledDateFilter] = useState(() => new Date().toISOString().split('T')[0])
  ...
  const fetchCancelledOrders = useCallback(async () => {
    try {
      const res = await fetch('/api/orders?status=cancelled', { headers: authHeaders(false) })
      if (handleFetchResponse(res) && res.ok) {
        const json = await res.json()
  ...
  useEffect(() => {
    if (showCancelled) fetchCancelledOrders()
  }, [showCancelled, fetchCancelledOrders])

  // Filter cancelled orders by date
  const filteredCancelled = cancelledDateFilter
    ? cancelledOrders.filter((o) => {
        const d = o.cancelledAt ?? o.createdAt
        return d.startsWith(cancelledDateFilter)
      })
    : cancelledOrders
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" onClick={() => { setShowCancelled(!showCancelled); if (!showCancelled) fetchCancelledOrders() }}>
            <XCircle className="size-4 mr-1" />
            {showCancelled ? 'Ver activos' : 'Cancelados'}
          </Button>
          <Button variant="outline" size="sm" onClick={fetchOrders}>
            <ShoppingCart className="size-4 mr-1" />Actualizar
          </Button>
        </div>
      </div>
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <div className="flex items-center gap-2">
              ...
              <Input type="date" value={cancelledDateFilter} onChange={(e) => setCancelledDateFilter(e.target.value)} className="h-9 w-40" />
            </div>
          </div>

          {filteredCancelled.length === 0 ? (
            <Card className="rounded-xl"><CardContent className="p-6 text-center"><XCircle className="size-12 mx-auto mb-3 text-muted-foreground opacity-30" /><p className="text-muted-foreground">No hay pedidos cancelados{cancelledDateFilter ? ' para esta fecha' : ''}</p></CardContent></Card>
          ) : (
            <Card className="rounded-xl"><CardContent className="p-0"><ScrollArea className="max-h-[70vh]"><table className="w-full text-sm"><thead><tr className="border-b bg-muted/50"><th className="text-left p-3">Mesa</th><th className="text-left p-3">Estado</th><th className="text-left p-3">Cancelado por</th><th className="text-left p-3">Items</th><th className="text-right p-3">Total</th><th className="text-left p-3">Fecha cancelación</th></tr></thead><tbody>{filteredCancelled.map((o) => (<tr key={o.id} className="border-b hover:bg-muted/30"><td className="p-3 font-medium">{o.table?.number ?? '?'}</td><td className="p-3"><Badge variant="outline" className="text-xs bg-red-100 text-red-800">Cancelado</Badge></td><td className="p-3 text-sm">{o.cancelledBy?.name || o.cancelledBy?.username || 'Usuario eliminado'}</td><td className="p-3">{o.items?.length ?? 0}</td><td className="p-3 text-right font-semibold">{formatEUR(o.total)}</td><td className="p-3 text-muted-foreground text-xs">{o.cancelledAt ? new Date(o.cancelledAt).toLocaleString('es-ES') : formatTime(o.createdAt)}</td></tr>))}</tbody></table></ScrollArea></CardContent></Card>
          )}
        </div>
      )}
    </div>
...
export default function RestaurantPage() {
  ...
  const isBarra = currentUser?.role === 'barra'
  ...
  const allTabs = [
    ...
    { id: 'barra' as TabId, label: 'Barra', icon: <Wine className="size-4" />, roles: ['super_admin', 'admin', 'encargado', 'barra'] },
  ...
  useEffect(() => {
    ...
    if (visibleTabs.length > 0 && !visibleTabs.find((t) => t.id === activeTab)) {
      ...
      else if (isBarra) setActiveTab('barra')
      else if (isCaja) setActiveTab('caja')
      else setActiveTab(visibleTabs[0].id)
    }
  }, [currentUser, visibleTabs, activeTab, setActiveTab, isSuperAdmin, isCamarero, isCocina, isBarra, isCaja])
