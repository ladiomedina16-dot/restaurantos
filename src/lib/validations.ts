...
// ─── Restaurant Settings / Fiscal Ticket ───────────────────

export const updateSettingsSchema = z.object({
  fiscalName: z.string().max(150).optional().default(''),
  taxId: z.string().max(50).optional().default(''),
  fiscalAddress: z.string().max(250).optional().default(''),
  phone: z.string().max(30).optional().default(''),
  email: z.string().email('Email inválido').optional().or(z.literal('')).default(''),
  ticketLegalText: z.string().max(500).optional().default(''),
  defaultVatRate: z.coerce.number().min(0).max(30).optional().default(21),
  logoUrl: z.string().optional().default(''),
  defaultDocumentType: z.enum(['ticket', 'factura']).optional().default('ticket'),
})
  // ─── Reports ───────────────────────────────────────────────
